...
def parse_ffmpeg_progress_line(
    line: str, total_duration_sec: Optional[float] = None
...
class TerminalProgressObserver:
    ...
    def on_progress(self, event: ProgressEvent) -> None:
        ...
        ch_str = (
            f"Ch {event.chapter:02d}"
            if event.chapter is not None and isinstance(event.chapter, int)
            else (f"Ch {event.chapter}" if event.chapter is not None else "")
        )
        ...
        self.progress.update(
            task_id, completed=event.percent, status=status_text, metrics=metrics_text
        )

        if (
            event.status == "COMPLETED" and event.percent >= 100.0
        ) or event.status == "ERROR":
            self.completed_jobs += 1
            pct = (
                int((self.completed_jobs / self.total_jobs) * 100)
