...
class USFMParser:
    ...
    def _clean_text(self, text):
        ...
        # Remove footnotes \f ... \f* (everything between including nested markers)
        text = re.sub(r'\\f\s+.*?\\f\*', '', text, flags=re.DOTALL)
        
        # Remove cross-references \x ... \x* (everything between including nested markers)
        ...
        for line in lines:
            ...
            if line.startswith('\\c '):
                ...
                if self.target_chapter:
                    ...
                    in_target_chapter = True  # Parse all chapters if no target specified
            ...
            if line.startswith(('\\id', '\\h', '\\toc', '\\mt', '\\imt', '\\im', 
                               '\\is', '\\io', '\\iot', '\\r', '\\ip')):
                continue
...
class SRTGenerator:
    ...
    def generate(self, start_counter=1):
        ...
        for verse_num, entries in verse_segments_map.items():
            ...
            has_segments = any(
                self._parse_segment_id(e['segment'])[1] is not None 
                for e in entries
            )
...
def convert_batch(usfm_file, timing_files, output_folder=None, combined=False):
    ...
    for i, timing_file in enumerate(timing_files, 1):
        ...
        if not timing_path.exists():
            print(f"  [{i}/{len(timing_files)}] [-] Timing file not found: {timing_path.name}")
            failed_count += 1
            continue
        ...
        try:
            ...
            if subtitle_count == 0:
                print(f"  [{i}/{len(timing_files)}] [!] Chapter {chapter_num}: 0 subtitles (timing file may not match USFM)")
                empty_count += 1
            else:
                print(f"  [{i}/{len(timing_files)}] [+] Chapter {chapter_num}: {subtitle_count} subtitles")
            
            success_count += 1
            
        except Exception as e:
            print(f"  [{i}/{len(timing_files)}] [-] Error processing {timing_path.name}: {e}")
            failed_count += 1
    ...
    if empty_count > 0:
        ...
        print(f"    (Timing files may be for different book than USFM)")
    print(f"{'='*60}")
