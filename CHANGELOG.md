...
### Fixed
- **Merged Verses (`\v 7-8`) Were Silently Absorbed Into the Previous Verse.** Two independent defects combined to lose whole verses from finished videos. First, the verse regex in `usfm_parser.py` accepted an ASCII hyphen only (`(\d+(?:-\d+)?)`), so a merged verse written with an **en-dash** — `\v 7–8`, which is what Paratext-style editors commonly produce — was not recognised as a verse start at all. The line fell through to the generic continuation branch and its text was appended to the *preceding* verse. Second, and independently, both copies of `_parse_segment_id` matched `^(\d+)([a-z])?$`, so a timing row identified as `7-8` resolved to `(None, None)` and rendered nothing — meaning even the ASCII-hyphen form, which did parse, could never reach the screen. Fixing only the first defect would have converted "text glued to the wrong verse" into "verse silently omitted".
  - Measured on the Mavilan Tulu Mark (`src/tcy-mav/42MRKTCYMAV.SFM`): 12 merged blocks across 9 of the 16 chapters (3, 4, 5, 6, 7, 9, 10, 12, 14; clean: 1, 2, 8, 11, 13, 15, 16). Mark 3's verse keys jumped from `6` straight to `11`, and verse 6 was **762 characters** carrying the text of verses 6 through 10 plus the literal strings `7–8` and `9–10` as visible subtitle content. After the fix it is 163 characters and every chapter's parsed verse count equals its `\v` count — that book-wide equality is the assertion worth re-running against any new translation.
  - **The heading mislabelling in Mark 3 and 4 was the same bug.** `align.py`'s `_heading_before_verse` keyed each heading by the raw `\v` token, so a heading anchored to `\v 7–8` never matched a parsed verse; `segments_from_usfm` then skipped it, and because `h_index` counts only *emitted* headings, every later `s`-number slid out of step with `USFMParser.sections`. Mark 3 timed 4 headings for 5 in the USFM and Mark 4 timed 5 for 6, so from the gap onward each heading displayed the *previous* section's title and the last one never appeared at all. Both chapters now emit the full set with each `s`-id mapping to its own heading.
  - Dash variants U+002D, U+2010–U+2015 all normalise to one canonical ASCII key, so `\v 7-8` and `\v 7–8` are the same verse. Verse references now render the full range (`3:7-8`) rather than a single number. The ASCII form is not an arbitrary choice: SAB's own reference timings for this book contain `7-8a`, `7-8b`, `9-10a`, `26-27f` and so on, so **every SAB-supplied timing file with a merged verse has always had rows VIDX silently declined to render** — the second defect above. Any project that took timings from SAB rather than `--align` was losing the same verses.
  - Re-aligned the 9 affected chapters and scored the whole book against the SAB references afterwards: median verse-start error 0.16s, p90 0.69s, 88% within 0.5s over **1747** matched starts, against a 0.16s/89% pre-fix baseline. The recovered verses were added without disturbing existing boundaries. Post-fix invariant across all 16 chapters: `\v` token count == `len(parser.verses)` == distinct timed verse ids, and timed `s`-row count == `len(parser.sections)`.
  - Unrelated pre-existing quirk noted while scoring: 15 verse ids across the book are labelled `9a` by `--align` where SAB writes plain `9` (an unsplit verse still gets an `a` suffix). Same verse and same audio, so the scorer simply fails to pair them; it shows up in chapters this fix never touched.
  - **Existing timing files for affected chapters must be regenerated** with `--align`: they were produced from the mis-parsed text, so they carry no rows for the merged verses and have the shifted `s`-numbering baked in.

### Changed
- **`_parse_segment_id` de-duplicated.** It existed byte-identically in `vidx/ass_generator.py` and `vidx/usfm_parser.py`, which is how one regex came to be wrong in two places. Now a single module-level `parse_segment_id()` in `usfm_parser.py`, alongside `normalize_verse_id()`; both classes keep a one-line delegate so no call sites changed.

## [0.4.4] - 2026-08-04

### Fixed
- **`--align` Never Worked in Any Released `vidx.exe`.** The release workflow built the executable with `pyinstaller --onefile --name vidx --clean vidx_entry.py`, which ignores `vidx.spec` entirely — so none of the spec's `hiddenimports` or `datas` were bundled. The v0.4.2 and v0.4.3 uroman fixes were committed, tested and released, but were never applied to the published binary, which failed with `Cannot open file ...uroman/data/romanization-table.txt` followed by `ValueError: No alignable text after romanization`. Verified by downloading and running the released v0.4.3 exe. The workflow now builds from `vidx.spec` (78.6MB vs the previous 58.3MB — the difference is the missing data), and a new post-build step greps the binary for the uroman tables and fails the release if they are absent; that check rejects the shipped v0.4.3 exe and passes a spec-built one. `vidx.spec` now also aborts with an explanation if the optional extras are missing at build time rather than silently producing a crippled executable.
  - **Publishing was not affected.** `vidx.spec` also bundles the google API discovery documents, but PyInstaller's own `googleapiclient` contrib hook collects those regardless of the spec, so they were present in the earlier exes too (verified by inspecting the published v0.4.3 binary). Only `--align` was broken.
  - Corrected `docs/alignment_guide.md` and the end-user cheat sheet, both of which stated the aligner had been bundled since v0.4.1.

### Changed
- **`scripts/benchmark_align.py`** now writes the `\separators` header that the CLI emits, so its output diffs cleanly against real `--align` output.
...
### Fixed
- **Published Wheels Contained No Code:** `pyproject.toml` declared no packages, so setuptools' auto-discovery saw the `src/` directory (scripture data, not Python) and assumed a src-layout project. Finding no package under it, it built wheels containing nothing but `dist-info` metadata. Both `pip install vidx-*.whl` and `pip install -e .` therefore installed a `vidx` console script that immediately failed with `ModuleNotFoundError: No module named 'vidx'`; only running from the repository root worked, because the current directory happened to be on `sys.path`. The `.whl` and `.tar.gz` assets attached to the v0.4.0, v0.4.1 and v0.4.2 GitHub releases are affected and should not be used — the bundled `vidx.exe` was never affected, since PyInstaller reads the source tree directly. Fixed by declaring `packages = ["vidx"]`.
