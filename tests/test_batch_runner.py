...
def test_batch_runner_generate_only_srt(sample_workspace):
    ...
    config = Config(
        config_dict={"project": {"generate_only": True, "subtitle_format": "srt"}}
    )
...
def test_batch_runner_generate_only_both(sample_workspace):
    ...
    config = Config(
        config_dict={"project": {"generate_only": True, "subtitle_format": "both"}}
    )
    runner = BatchRunner(config)
    runner.generate_only = True
...
def test_preprocess_background_media_loop_crossfade(monkeypatch, tmp_path):
    ...
    runner = BatchRunner(config)
    runner.add_job(
        "a.sfm", "t.txt", "audio.mp3", "out.mp4", background_media=str(bg_file)
    )
